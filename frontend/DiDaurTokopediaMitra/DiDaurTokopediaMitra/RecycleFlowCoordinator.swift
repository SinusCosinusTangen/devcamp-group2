...
public enum RecycleFlowCoordinatorInstructor {
    ...
    case detail(requestValue: REDetailViewModelRequestValue)
}
...
extension DefaultRecycleFlowCoordinator: RecycleFlowCoordinator {
    ...
    public func start(with instructor: RecycleFlowCoordinatorInstructor) {
        ...
        case .success(let requestValue):
            ...
            self.navigationController.pushViewController(vc, animated: true)
        case .detail(let requestValue):
            let vc = initREDetailController(requestValue: requestValue)
            self.navigationController.pushViewController(vc, animated: true)
        }
    }
}
...
private extension DefaultRecycleFlowCoordinator {
    ...
    func initREDetailController(requestValue: REDetailViewModelRequestValue) -> REDetailController {
        let route = REDetailViewModelRoute(start: self.startRecycleFlow(_:))
        return self.viewControllerFactory.makeREDetailController(requestValue: requestValue, route: route)
    }
}
