...
public protocol REDashboardViewModelInput {
    ...
    func didSelected(result: ResultDomain)
}
...
public protocol REDashboardViewModelOutput {
    ...
    var listItem: PublishRelay<[ResultDomain]> { get set }
}
...
public final class DefaultREDashboardViewModel: REDashboardViewModel {
    ...
    public var listItem = PublishRelay<[ResultDomain]>()
...
public extension DefaultREDashboardViewModel {
    ...
    func didSelected(result: ResultDomain) {
        let requestValue = REDetailViewModelRequestValue(result: result)
        self.route.start(.detail(requestValue: requestValue))
    }
...
private extension DefaultREDashboardViewModel {
    ...
    func doFetchListUseCase() {
        let requestValue = FetchListUseCaseRequest()
        
        self.fetchListUseCase.execute(requestValue).subscribe { [weak self] apiResult in
            guard let self = self else { return }
            switch apiResult {
            case .success(let response):
                guard let response = response else { return }
                self.listItem.accept(response.result)
            case .failure(let errorDomain):
                let error = NSError.create(with: errorDomain.details?.joined(separator: " ") ?? "Failed to fetch user data")
                print(error)
            }
